...
// Canonical 31-row status list. The first 8 rows are the doorstep-pickup
// sub-flow (`phase: 'PICKUP'`); the remaining rows are the in-shop service
// flow (`phase: 'SERVICE'`). The renderer accepts an optional `phaseFilter`
// prop to drop one phase — used so the owner's Service History screen hides
// pickup rows while the customer's order tracker keeps the full timeline.
//
// `value` keeps the DB-stored codes — codename renames (e.g.
// PICKUP_ON_THE_WAY → PICKUP_PERSON_ON_THE_WAY) ship in a separate change
// alongside a DB migration. Labels here are the display strings the user
// sees and are kept aligned with the customer-facing canonical table.
//
// Codes that the backend still emits but that we hide from the user-facing
// timeline (INVOICE_READY, DELIVERED_PROCESSING) are omitted
// from this list. They keep flowing through repair_booking_events; the
// rendered timeline just doesn't show a dedicated row for them.
export const SHOP_BOOKING_STATUS_OPTIONS = [
  ...
  // One spare-parts row. PARTS_REPLACED ("Spare Parts Replaced") was retired by
  // migration 87, which also deleted its history rows.
  { value: 'PARTS_REQUIRED',                                label: 'Spare Parts Waiting',                   phase: 'SERVICE' },
  // One quality-check row. QUALITY_CHECK_STARTED was retired by migration 88,
  // which also deleted its history rows.
...
export function ServiceHistoryTimeline({ events, status, phaseFilter }) {
  ...
  // No filter = full canonical list (customer order tracker).
  const filteredRows = phaseFilter
  ...
  let currentIndex = filteredRows.findIndex((o) => o.value === latestKey);
  if (currentIndex < 0) {
    // Latest event isn't in the visible row set — fall back to the highest
    // indexed visible step that does have an event so we still mark something
    // current within whatever phase the caller is showing.
    filteredRows.forEach((opt, i) => {
  ...
  return (
    <View>
      {filteredRows.map((opt, i) => {
        ...
        const nextOpt = filteredRows[i + 1];
