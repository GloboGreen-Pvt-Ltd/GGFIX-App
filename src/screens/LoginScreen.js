...
export default function LoginScreen({ onLogin, navigation }) {
  ...
  // Maps the entered identifier to the customer-login body: mobile OR email.
  const credentialId = () => (isPhone ? { mobile: phoneDigits } : { email: trimmedId });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const data = await customerLogin({ ...credentialId(), ...credential });
      onLogin(data);
    } catch (e) {
      ...
      } else if (/unauthorized|invalid/i.test(String(msg))) {
        setError('Wrong credentials, or no account for that mobile/email. Tap "Create account" to sign up.');
      } else {
        setError(msg);
      }
    } finally {
  ...
  const renderError = () =>
    error ? (
      ...
      >
        <Text style={{ fontSize: rf(12.5), color: '#B91C1C', lineHeight: rlh(18) }}>{error}</Text>
      </View>
    ) : null;
        ...
        >
          ...
          >
            ...
            <View style={{ alignItems: 'center', marginBottom: isShort ? 14 : 20 }}>
              ...
              <Text style={{ fontSize: rf(20), fontWeight: '800', color: GREEN, letterSpacing: -0.3 }}>
                GGFIX - Customer
              </Text>
              <Text style={{ fontSize: rf(12), color: MUTED, marginTop: 3 }}>
            ...
            {step === 'IDENTIFIER' ? (
              /* ---------- Step 1: identifier ---------- */
              <>
                <Text style={{ fontSize: rf(13), color: MUTED, marginTop: 4, lineHeight: rlh(19) }}>
                ...
                <View style={fieldRow}>
                  {isPhone && (
                    <View style={ccChip}>
                      <Text style={{ fontSize: rf(14), fontWeight: '700', color: INK }}>🇮🇳 +91</Text>
                    </View>
                  )}
                  <Input
                ...
                <Button
                  ...
                  textClassName="font-extrabold tracking-wide"
                >
                  Continue
                </Button>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginTop: 20 }}>
                  ...
                  <Pressable onPress={() => navigation?.navigate('CreateAccount')} hitSlop={8}>
                    <Text style={{ fontSize: rf(13), fontWeight: '800', color: GREEN }}>Create account</Text>
                  </Pressable>
                </View>
              </>
            ) : (
              ...
              <>
                ...
                >
                  <Text style={{ flex: 1, fontSize: rf(14), fontWeight: '700', color: INK }} numberOfLines={1}>
                    {prettyId}
                  </Text>
                  <Pressable onPress={backToIdentifier} hitSlop={8}>
                    <Text style={{ fontSize: rf(13), fontWeight: '800', color: GREEN }}>Change</Text>
                  </Pressable>
                </View>
                ...
                {mode === 'PASSWORD' ? (
                  <>
                    <View style={labelRow}>
                      ...
                      <Pressable
                        onPress={() => navigation?.navigate('ForgotPassword', { identifier: isPhone ? phoneDigits : trimmedId })}
                        hitSlop={8}
                      >
                        <Text style={{ fontSize: rf(12.5), fontWeight: '700', color: GREEN }}>Forgot password?</Text>
                      </Pressable>
                    </View>
                    <View style={fieldRow}>
                  ...
                  <>
                    <View style={labelRow}>
                      ...
                      <Pressable onPress={handleSendOtp} hitSlop={8}>
                        <Text style={{ fontSize: rf(12.5), fontWeight: '700', color: GREEN }}>Send code</Text>
                      </Pressable>
                    </View>
                    <View style={fieldRow}>
                    ...
                    {otpHint ? (
                      <Text style={{ fontSize: rf(12), color: GREEN, marginTop: 8 }}>{otpHint}</Text>
                    ) : (
                      <Text style={{ fontSize: rf(12), color: MUTED, marginTop: 8 }}>
                        Tap “Send code”. {isPhone ? 'Default OTP is 123456.' : 'A code will be emailed to you.'}
                      </Text>
                    )}
                  </>
                )}
            ...
            <Text style={{ fontSize: rf(11), color: MUTED, textAlign: 'center', marginTop: 18, lineHeight: rlh(16) }}>
...
function OrDivider() {
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', marginVertical: 16 }}>
      ...
      <Text style={{ marginHorizontal: 12, fontSize: rf(12), color: MUTED, fontWeight: '600' }}>or</Text>
      <View style={{ flex: 1, height: 1, backgroundColor: FIELD_BORDER }} />
    </View>
  );
}
...
function OutlineButton({ onPress, icon, label }) {
  return (
    ...
    >
      {icon}
      <Text style={{ marginLeft: 8, fontSize: rf(14.5), fontWeight: '700', color: GREEN }}>{label}</Text>
    </Pressable>
  );
}
