  ...
  return (
    <AuthShell onBack={goBack}>
      ...
      <View style={{ marginTop: 26 }}>
        <Text style={{ fontSize: rf(14), fontWeight: '800', color: INK, marginBottom: 8 }}>Secure password tips</Text>
        {TIPS.map((tip, i) => (
          <View key={i} style={{ flexDirection: 'row', marginBottom: 6 }}>
            <Text style={{ fontSize: rf(13), color: MUTED, marginRight: 6 }}>•</Text>
            <Text style={{ flex: 1, fontSize: rf(12.5), color: MUTED, lineHeight: rlh(18) }}>{tip}</Text>
          </View>
        ))}
      </View>
    </AuthShell>
