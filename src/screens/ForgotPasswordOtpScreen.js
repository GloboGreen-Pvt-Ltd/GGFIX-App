    ...
    <AuthShell onBack={goBack}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 12 }}>
        <Pressable onPress={resend} disabled={seconds > 0} hitSlop={8}>
          <Text style={{ fontSize: rf(13), fontWeight: '700', color: seconds > 0 ? MUTED : GREEN }}>Resend code</Text>
        </Pressable>
        {seconds > 0 ? (
          <Text style={{ fontSize: rf(12.5), color: MUTED, marginLeft: 8 }}>· wait {seconds}s</Text>
        ) : null}
      </View>

      {note ? <Text style={{ fontSize: rf(12.5), color: GREEN, marginTop: 8 }}>{note}</Text> : null}
      <ErrorBox msg={error} />
