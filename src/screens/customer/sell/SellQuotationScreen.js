...
const styles = StyleSheet.create({
  ...
  cell: { flex: 1, fontSize: rf(12), color: colors.text },
  link: { color: '#2563EB' },
});
...
export default function SellQuotationScreen({ route }) {
  ...
  return (
    <View style={styles.container}>
      <Text style={{ fontSize: rf(15), fontWeight: '700', color: colors.text, marginBottom: 8 }}>Total Quotation : {items.length}</Text>
