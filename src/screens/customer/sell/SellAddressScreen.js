  ...
  return (
    <View className="flex-1 bg-background">
      <ScrollView contentContainerStyle={{ padding: 12, paddingBottom: bottomSpace }}>
        <View className="mb-2">
          <Text className="font-extrabold text-text" style={{ fontSize: rf(15) }}>Where should we pick up?</Text>
          <Text className="text-text-muted mt-0.5" style={{ fontSize: rf(11) }}>Free doorstep pickup across serviceable areas.</Text>
        </View>
        ...
        >
          ...
          <Text className="font-extrabold text-primary ml-1.5" style={{ fontSize: rf(12) }}>Add new address</Text>
        </Pressable>
                ...
                <View className="flex-row items-start">
                  ...
                  <View className="flex-1">
                    <View className="flex-row items-center">
                      <Text className="font-extrabold text-text mr-1.5" style={{ fontSize: rf(13) }}>{a.label || 'Address'}</Text>
                      {a.isDefault ? <Badge variant="softSuccess">DEFAULT</Badge> : null}
                    </View>
                    {(a.fullName || a.mobile) ? (
                      <Text className="text-text-muted mt-0.5" style={{ fontSize: rf(11) }} numberOfLines={1}>{a.fullName}{a.mobile ? ` · ${a.mobile}` : ''}</Text>
                    ) : null}
                    <Text className="text-text mt-0.5 leading-4" style={{ fontSize: rf(11) }} numberOfLines={2}>
