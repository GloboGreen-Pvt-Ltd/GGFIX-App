      ...
      <View className="p-4">
        <View className="flex-row items-start">
          ...
          <View className="flex-1 pr-2">
            <Text numberOfLines={1} className="font-extrabold text-text" style={{ fontSize: rf(15) }}>
              {device || 'Device'}
            </Text>
            <Text numberOfLines={1} className="text-text-muted mt-0.5" style={{ fontSize: rf(11.5) }}>
          ...
          <View className="mt-3 pt-3 border-t border-border">
            {scheduledAt ? (
              <View className="flex-row items-center mb-1.5">
                ...
                <Text className="ml-2 text-text-muted" style={{ fontSize: rf(12) }} numberOfLines={1}>{scheduledAt}</Text>
              </View>
            ) : null}
            {address ? (
              <View className="flex-row items-center mb-1.5">
                <MapPin size={13} color={tokens.textMuted} />
                <Text className="ml-2 flex-1 text-text-muted" style={{ fontSize: rf(12) }} numberOfLines={1}>{address}</Text>
              </View>
            ) : null}
            {amount != null ? (
              <View className="flex-row items-center justify-between mt-1">
                <View className="flex-row items-center">
                  ...
                  <Text className="ml-0.5 font-extrabold text-text" style={{ fontSize: rf(14) }}>{amount}</Text>
                  <Text className="ml-2 text-text-muted" style={{ fontSize: rf(11) }}>{amountCaption}</Text>
                </View>
                {rightLabel ? (
                  <Pressable onPress={onRightPress || onPress} className="flex-row items-center active:opacity-70">
                    <Text className="font-bold text-primary mr-0.5" style={{ fontSize: rf(12) }}>{rightLabel}</Text>
