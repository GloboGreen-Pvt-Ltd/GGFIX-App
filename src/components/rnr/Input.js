  ...
  if (!leftIcon && !rightIcon) {
    return (
      <TextInput
        ...
        className={cn(
          'bg-card rounded-2xl px-4 py-3 text-text',
  ...
  return (
    ...
    >
      ...
      <TextInput
        ...
        style={[WEB_NO_OUTLINE, { paddingVertical: 6, fontSize: rf(16) }, style]}
      />
...
export function Label({ className, children, required }) {
  return (
    <Text className={cn('font-bold text-text mb-2', className)} style={{ fontSize: rf(13) }}>
      {children}
...
export function FormField({ label, required, error, hint, children, className }) {
  return (
    <View className={cn('mb-4', className)}>
      ...
      {children}
      {error ? (
        <Text className="text-danger mt-1" style={{ fontSize: rf(12) }}>{error}</Text>
      ) : hint ? (
        <Text className="text-text-muted mt-1" style={{ fontSize: rf(12) }}>{hint}</Text>
      ) : null}
    </View>
  );
}
