  ...
  return (
    ...
    >
      ...
      <View className="flex-1 pr-2">
        <Text className={cn('font-bold', destructive ? 'text-danger' : 'text-text')} style={{ fontSize: rf(14) }} numberOfLines={1}>
          {title}
        </Text>
        {subtitle ? (
          <Text className="text-text-muted mt-0.5" style={{ fontSize: rf(12) }} numberOfLines={2}>{subtitle}</Text>
        ) : null}
      </View>
      {right}
